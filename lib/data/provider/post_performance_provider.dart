...
class PostPerformanceProvider with ChangeNotifier {
  ...
  late PostModel _postPerformance;
  PostPerformanceState _state = PostPerformanceState.none;
  String? error;

  PostModel get postPerformance => _postPerformance;
  PostPerformanceState get state => _state;

  // dummy data
  Future<void> fetchPostPerformanceById(String postId) async {
  ...
  notifyListeners();
    ...
    //   document: Queries.getPostPerformanceById,
    ...
    final queryOption = QueryOptions(
      ...
      fetchPolicy: FetchPolicy.networkOnly,
      variables: {
        'post_id': 12345, //TODO fetch real user id
      });
    ...
    try {
      ...
      if (postPerformanceData != null) {
        _postPerformance = PostModel.fromJson(postPerformanceData);
        _state = PostPerformanceState.loaded;
      } else {
